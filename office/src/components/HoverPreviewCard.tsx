...
function parseStatusLine(raw: string): StatusLineParsed | null {
  ...
  // Format with bars: 📡 ████░░░░░░ 26% 261k/1000k • 5h████░░13% • 7d██░░25% • 75h58m • Opus 4.6 (1M context) • 320b412f on main
  // Format without bars: 📡 42% 83k/200k • 4h13m • Opus 4.6 • 70d6aff3 on main*
  // Strip bar chars (█░) before parsing
  const clean = line.replace(/[█░]/g, "").replace(/\s+/g, " ");
  const m = clean.match(/📡\s+(\d+)%\s+(\d+k\/\d+k)\s+•\s+(?:5h\s*\d+%\s+•\s+7d\s*\d+%\s+•\s+)?(\S+)\s+•\s+(.+?)\s+•\s+(\w+)(?:\s+on\s+(.+))?/);
  if (m) {
    return {
...
export const HoverPreviewCard = memo(function HoverPreviewCard({
  ...
  // Pause content updates while user is selecting text
  const selectingRef = useRef(false);
  useEffect(() => {
    const onSelectStart = () => { selectingRef.current = true; };
    const onSelectEnd = () => { selectingRef.current = false; };
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (active && !selectingRef.current && !hasSelection) setContent(data.content || "");
      } catch {}
  ...
  // which destroys text selection on every poll cycle. (ported from 1a0f985)
  ...
  return (
    <div
      ...
      onMouseDown={(e) => {
        if (pinned && e.target !== inputRef.current && !termRef.current?.contains(e.target as Node)) {
          e.preventDefault();
          inputRef.current?.focus();
        }
      }}
    >
      ...
      <div className="relative flex-1" style={{ background: "#08080c" }}>
        <div
          ...
          style={{ touchAction: "pan-y", overscrollBehavior: "contain", userSelect: "text", WebkitUserSelect: "text", cursor: "text" }}
        />
        {pinned && send && (
