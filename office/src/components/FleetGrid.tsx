...
export const FleetGrid = memo(function FleetGrid({
  ...
  // --- Oracle project assignments (poll every 30s) ---
  type OracleProjectData = {
    assignments: Record<string, { projectId: string; source: string; updatedAt: string }>;
    activeProjects: { id: string; name: string }[];
  };
  const [oracleProjects, setOracleProjects] = useState<OracleProjectData | null>(null);
  useEffect(() => {
    let cancelled = false;
    const fetchProjects = () => {
      fetch("/api/oracle-projects").then(r => r.ok ? r.json() : null).then(data => {
        if (!cancelled && data) setOracleProjects(data);
      }).catch(() => {});
    };
    fetchProjects();
  ...
  const handleProjectChange = useCallback((oracle: string, projectId: string) => {
    fetch("/api/oracle-projects", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ oracle, projectId: projectId || null }),
    }).then(r => r.ok ? r.json() : null).then(() => {
      // Optimistic update
      setOracleProjects(prev => {
        if (!prev) return prev;
        const next = { ...prev, assignments: { ...prev.assignments } };
        if (projectId) {
          next.assignments[oracle] = { projectId, source: "manual", updatedAt: new Date().toISOString() };
        } else {
          delete next.assignments[oracle];
        }
        return next;
      });
    }).catch(() => {});
              ...
              {recentlyActive.map((entry, i) => {
                ...
                return (
                  <AgentRow key={`recent-${entry.target}`} agent={agent} accent={rs.accent} roomLabel={rs.label}
                    ...
                    oracleProjects={oracleProjects} onProjectChange={handleProjectChange} />
                );
              })}
            </div>
          )}
