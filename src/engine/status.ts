...
export class StatusDetector {
  ...
  private advisoryTimers = new Map<string, number>();
  ...
  async detect(
    ...
    for (const { target, name, session } of agents) {
      ...
      if (isAgent) {
        ...
        // T011 + T036: capture claude pane for advisory + thinking detection
        try {
          const pane = await capture(target, 15);
          const plain = pane.replace(/\x1b\[[0-9;]*[a-zA-Z]/g, "");

          // T050: detect ACTIVE work only (paren-pattern), not stale "for Ns" history
          // ❯ prompt is always visible in Claude Code — discriminates nothing
          const hasThinkingGlyph = plain.split('\n').map((l: string) => l.trim()).some((l: string) =>
          ...
          const newStatus = hasThinkingGlyph ? "busy" : (prevStatus === "busy" ? "ready" : prevStatus);
          if (newStatus !== prevStatus) {
            this.state.set(target, { hash: "", changedAt: now, status: newStatus, wasRunning: true });
            const busyMsg = JSON.stringify({ type: "recent", agents: hasThinkingGlyph ? [{ target, name, session }] : [] });
            for (const ws of clients) { try { ws.send(busyMsg); } catch {} }
          }

          // T011: Advisory auto-recovery
          const hasStatusBar = /📡/.test(plain) || /\d+%\s+\d+k\/\d+k/.test(plain);
          const isAdvisory = /share one limit|% of usage|Skills|usage advisory|rate limit/i.test(plain);
          const isPrompt = /\(y\/n\)|approve|deny|allow|permission|Are you sure/i.test(plain);
          if (!hasStatusBar && isAdvisory && !isPrompt) {
            const advisoryKey = `advisory:${target}`;
            const firstSeen = this.advisoryTimers.get(advisoryKey) || now;
            if (!this.advisoryTimers.has(advisoryKey)) this.advisoryTimers.set(advisoryKey, now);
            if (now - firstSeen > 120_000) {
              const { sendKeys } = await import("../ssh");
              await sendKeys(target, "\x1b");
              await new Promise(r => setTimeout(r, 500));
              await sendKeys(target, "\r");
              this.advisoryTimers.delete(advisoryKey);
              const { appendFileSync } = await import("fs");
              const { join } = await import("path");
              const { homedir } = await import("os");
              const ts = new Date().toISOString().replace("T", " ").slice(0, 19);
              appendFileSync(join(homedir(), ".oracle/feed.log"), `${ts} | SYSTEM | local | Notification | SYSTEM | auto-dismissed advisory on ${target}\n`);
            }
          } else {
            this.advisoryTimers.delete(`advisory:${target}`);
          }
        } catch {}
