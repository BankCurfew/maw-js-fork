...
/** After fleet spawn, send /recap to ALL oracles so they read threads, focus, handoffs */
async function recapAllOracles(fleet: FleetSession[]) {
  console.log("  \x1b[36mSending /recap to all oracles...\x1b[0m\n");
  let sent = 0;
  for (const sess of fleet) {
    ...
    for (const win of sess.windows) {
      try {
        ...
        if (!/claude|node/i.test(paneCmd)) continue;
        // Give Claude time to boot
        await new Promise(r => setTimeout(r, 3000));
        await tmux.sendText(`${sess.name}:${win.name}`, `/recap`);
        console.log(`  \x1b[32m↻\x1b[0m ${win.name}: /recap sent`);
        sent++;
      } catch { /* window not ready */ }
    }
  }
  // Also recap shell session (operator's entry point, not in fleet configs)
  try {
    const paneCmd = await ssh("tmux display-message -t shell:shell -p '#{pane_current_command}' 2>/dev/null");
    if (/claude|node/i.test(paneCmd)) {
      await new Promise(r => setTimeout(r, 3000));
      await tmux.sendText("shell:shell", "/recap");
      console.log("  \x1b[32m↻\x1b[0m shell: /recap sent");
      sent++;
    }
  } catch { /* shell not ready */ }
  console.log(`\n  \x1b[32m${sent} oracles sent /recap.\x1b[0m\n`);
}
...
export async function cmdWakeAll(opts: { kill?: boolean; all?: boolean; resume?: boolean; recapAll?: boolean } = {}) {
  ...
  // Skip dormant (30+) unless --all flag is passed
  // Active range: 01-29 (core + extended oracles), 99 (special)
  const sessions = opts.all
    ? allSessions
    : allSessions.filter(s => {
        const num = parseInt(s.name.split("-")[0], 10);
        return isNaN(num) || num < 30 || num >= 99;
      });
  ...
  // Ensure large canvas so Claude Code redraws enough rows for web dashboard capture
  await ssh("tmux set-option -g default-size 200x50 2>/dev/null").catch(() => {});
  ...
  for (const sess of sessions) {
    ...
    try {
      ...
      for (const win of sess.windows) {
        try {
          const paneCmd = await ssh(`tmux display-message -t '${sess.name}:${win.name}' -p '#{pane_current_command}' 2>/dev/null`);
          if (!/claude|node/i.test(paneCmd)) {
            // Claude not running in this window — clear history, cd to repo, relaunch
            if (!sess.skip_command) {
              const winPath = `${loadConfig().ghqRoot}/${win.repo}`;
              await ssh(`tmux clear-history -t '${sess.name}:${win.name}' 2>/dev/null`).catch(() => {});
              await ssh(`tmux send-keys -t '${sess.name}:${win.name}' 'cd ${winPath}' Enter`);
    ...
    await ssh(`tmux resize-window -t '${sess.name}' -x 200 -y 50 2>/dev/null`).catch(() => {});
  ...
  // Ensure "shell" tmux session for operator Command room
  try {
    const shellCheck = (await ssh("tmux has-session -t shell 2>/dev/null && echo yes || echo no")).trim();
    if (shellCheck === "no") {
      await ssh(`tmux new-session -d -s shell -n shell -c ${process.env.HOME || "/tmp"}`);
      await ssh("tmux set-environment -t shell ORACLE_NAME shell");
      for (const [key, val] of Object.entries(getEnvVars())) {
        await ssh(`tmux set-environment -t shell '${key}' '${val}'`);
      }
      await ssh(`tmux send-keys -t shell:shell '${buildCommand("shell")}' Enter`);
      // Pin window name after Claude starts (Claude process renames it)
      await ssh("tmux set-option -t shell:shell allow-rename off");
      console.log("  \x1b[32m●\x1b[0m shell — Claude Code session created (Command room)");
    } else {
      // Ensure window name stays "shell" even if Claude renamed it
      try {
        const winName = (await ssh("tmux display-message -t shell -p '#{window_name}'")).trim();
        if (winName !== "shell") {
          await ssh("tmux rename-window -t shell 'shell'");
          await ssh("tmux set-option -t shell:shell allow-rename off");
        }
      } catch { /* ok */ }
  ...
  if (opts.recapAll) {
    // --recap-all: send /recap to ALL oracles (reads threads, focus, handoffs)
    await recapAllOracles(sessions);
  } else if (opts.resume) {
    // --resume: only send /recap to oracles with active Pulse tickets
