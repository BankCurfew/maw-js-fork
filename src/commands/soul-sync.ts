...
import { ssh as hostExec } from "../ssh";
 ...
 *      repoRoot = `/home/neo/Code/github.com/maw-js`
 *      → rel = `maw-js` → slug = `maw-js`
 *
 *   B. bare ghq root:     `/home/neo/Code`
 *      repoRoot = `/home/neo/Code/github.com/maw-js`
 *      → rel = `github.com/maw-js` → (strip host) →
 *        `maw-js` → slug = `maw-js`
 *
 * Before this normalization, shape B produced the org-only slug
 * `github.com/YourOrg` because `.split("/").slice(0, 2)` grabbed
