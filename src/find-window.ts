...
export function findWindow(sessions: Session[], query: string): string | null {
  const q = ORACLE_ALIASES[query.toLowerCase()] || query.toLowerCase();
  ...
  // Strict session match first (exact + oracle-name, no substring)
  // Prevents "dev" matching "botdev", "aia" matching "admin", etc.
  const strictSess = matchSession(sessions, q, true);
  if (strictSess && strictSess.windows.length > 0) {
    return `${strictSess.name}:${strictSess.windows[0].index}`;
  }

  // Match window names (substring)
  for (const s of sessions) {
    for (const w of s.windows) {
      if (w.name.toLowerCase().includes(q)) return `${s.name}:${w.index}`;
    }
  }
  // Match session names — return first window of matching session (substring)
  for (const s of sessions) {
