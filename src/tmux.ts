...
export class Tmux {
  ...
  async resizeWindow(target: string, cols: number, rows: number): Promise<void> {
    await this.tryRun("resize-window", "-t", target, "-x", String(cols), "-y", String(rows));
  }
  ...
  async listPaneIds(): Promise<Set<string>> {
    try {
      const raw = await this.tryRun("list-panes", "-a", "-F", "#{pane_id}");
  ...
  async listPanes(): Promise<TmuxPane[]> {
    try {
      const raw = await this.tryRun("list-panes", "-a", "-F", "#{pane_id}:#{session_name}:#{window_index}:#{pane_current_command}:#{pane_current_path}");
      return raw.split("\n").filter(Boolean).map(line => {
        const [id, session, windowIndex, command, cwd] = line.split(":");
        return { id, session, windowIndex: +windowIndex, command, cwd };
      });
    } catch { return []; }
  }
