...
describe("Tmux", () => {
  ...
  describe("newWindow", () => {
    test("basic new-window command", async () => {
    ...
    test("includes session target and window name", async () => {
      await t.newWindow("neo", "neo-mqtt-feed", { cwd: "/tmp/wt" });
      expect(commands[0]).toBe("tmux new-window -t neo -n neo-mqtt-feed -c /tmp/wt 2>/dev/null");
    });
  });
  ...
  describe("run", () => {
    ...
    test("appends stderr suppression", async () => {
      let captured = "";
      ...
      expect(captured).toBe("tmux has-session -t neo 2>/dev/null");
      expect(captured).toContain("2>/dev/null");
    });
  });
