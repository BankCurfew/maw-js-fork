...
describe("soul-sync", () => {
  ...
  describe("project_repos lookup (cell membrane)", () => {
    test("findProjectsForOracle returns configured project_repos", () => {
      ...
      const fleet = [
        { name: "08-mawjs", windows: [], project_repos: ["YourOrg/maw-js"] },
        { name: "01-pulse", windows: [] },
      ];
      expect(findProjectsForOracleForTest("mawjs", fleet)).toEqual(["YourOrg/maw-js"]);
    ...
    test("findOracleForProject returns owning oracle name", () => {
      ...
      const fleet = [
        { name: "08-mawjs", windows: [], project_repos: ["YourOrg/maw-js"] },
        { name: "01-pulse", windows: [], project_repos: ["YourOrg/example", "YourOrg/example"] },
      ];
      expect(findOracleForProjectForTest("YourOrg/maw-js", fleet)).toBe("mawjs");
      expect(findOracleForProjectForTest("YourOrg/example", fleet)).toBe("pulse");
      expect(findOracleForProjectForTest("nobody/orphan", fleet)).toBeNull();
    });

    test.skip("resolveProjectSlug handles github.com-rooted ghqRoot (#193 — hostExec removed)", () => {
      const { resolveProjectSlug } = require("../src/commands/soul-sync");
      const ghqRoot = "/home/neo/Code/github.com";
      expect(
        resolveProjectSlug("/home/neo/Code/github.com/YourOrg/maw-js", ghqRoot)
      ).toBe("YourOrg/maw-js");
    });

    test.skip("resolveProjectSlug handles bare ghq root (#193 — hostExec removed)", () => {
      const { resolveProjectSlug } = require("../src/commands/soul-sync");
      const ghqRoot = "/home/neo/Code";
      // Before the fix this returned "github.com/YourOrg" (org-only) —
    ...
    test.skip("resolveProjectSlug strips worktree suffix (#193 — hostExec removed)", () => {
      const { resolveProjectSlug } = require("../src/commands/soul-sync");
      const ghqRoot = "/home/neo/Code/github.com";
      expect(
        resolveProjectSlug("/home/neo/Code/github.com/YourOrg/maw-js.wt-issue-193", ghqRoot)
      ).toBe("YourOrg/maw-js");
    });

    test.skip("resolveProjectSlug returns null when repoRoot is outside ghqRoot (#193 — hostExec removed)", () => {
      const { resolveProjectSlug } = require("../src/commands/soul-sync");
      expect(
    ...
    test.skip("resolveProjectSlug returns null when path has no repo segment (#193 — hostExec removed)", () => {
      const { resolveProjectSlug } = require("../src/commands/soul-sync");
      // Sitting at the org directory, no repo to resolve.
      expect(
        resolveProjectSlug("/home/neo/Code/github.com/YourOrg", "/home/neo/Code/github.com")
      ).toBeNull();
    });
  ...
  describe("findPeers (flat peer lookup)", () => {
    test("returns empty for oracle without sync_peers", () => {
      ...
      const fleet = [
        { name: "01-pulse", windows: [{ name: "pulse-oracle", repo: "YourOrg/pulse-oracle" }] },
        { name: "06-example", windows: [{ name: "example-oracle", repo: "YourOrg/example-oracle" }] },
      ];
      expect(findPeersForTest("example", fleet)).toEqual([]);
    });

    test("returns configured sync_peers", () => {
      const { findPeersForTest } = require("./soul-sync-helpers");
      const fleet = [
        { name: "06-example", windows: [], sync_peers: ["pulse", "neo"] },
        { name: "01-pulse", windows: [], sync_peers: ["example", "fireman"] },
      ];
      expect(findPeersForTest("example", fleet)).toEqual(["pulse", "neo"]);
      expect(findPeersForTest("pulse", fleet)).toEqual(["example", "fireman"]);
    });

    test("returns empty for oracle with empty sync_peers", () => {
      const { findPeersForTest } = require("./soul-sync-helpers");
      const fleet = [
        { name: "06-example", windows: [], sync_peers: [] },
      ];
      expect(findPeersForTest("example", fleet)).toEqual([]);
    });
  });
